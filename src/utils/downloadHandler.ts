...
export const pdfHandler = async (renderedMarkdown: any, mermaidInstance: any) => {
    ...
    const imageMap  = imageStore.getImageMap();
    // console.log('renderedMarkdown', renderedMarkdown.value);
    let replacedMarkdown = renderedMarkdown.value; // 复制原始 Markdown 内容
    Object.entries(imageMap).forEach(([onlineLink, base64Link]) => {
        // 将所有在线链接替换为 Base64 链接（全局替换）
        replacedMarkdown = replacedMarkdown.replaceAll(onlineLink, base64Link);
    });

    // console.log('replacedMarkdown', replacedMarkdown);
    const tempElement = document.createElement('div');
    tempElement.innerHTML = replacedMarkdown;
    // 异步渲染 Mermaid 图表
