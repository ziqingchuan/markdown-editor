...
// 移除同步导入的mammoth和pdfjsLib，改为动态导入
...
export const FileUploadHandler = {
    async handleFile(file: File): Promise<string> {
        ...
        switch (extension) {
            ...
            case 'md':
                return this.parseMarkdown(file);
            case 'pdf':
                return this.parsePDF(file);  // 动态加载pdfjs-dist
            case 'txt':
                return this.parseText(file);
            case 'docx':
            case 'doc':
                return this.parseDoc(file);  // 动态加载mammoth
            default:
                throw new Error('不支持的文件类型');
        }
    },
    ...
    // PDF解析：动态加载pdfjs-dist
    async parsePDF(file: File): Promise<string> {
        try {
            // 按需加载pdfjs-dist核心库和worker
            const [pdfjsLibModule] = await Promise.all([
                import('pdfjs-dist'),
            ]);
