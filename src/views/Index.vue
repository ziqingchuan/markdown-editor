      ...
      <div v-show="!onlyEdit" class="preview-container" :class="{ 'only-preview': onlyPreview }">
        <div class="panel-header preview-header">
          ...
          <Tooltip
              ...
              style="margin-left: auto"
          >
            <div class="upload-btn" @click="isHtmlMode = !isHtmlMode">
              <svg class="upload-icon" :class="{'blue': isHtmlMode}" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" width="16" height="16"><path d="M772.614973 574.420953h248.357936c1.949817-17.207135 2.680998-34.804233 2.680998-52.84004a460.351786 460.351786 0 0 0-62.199161-231.735746c40.897411-108.702296 39.483794-201.026129-15.354808-256.157204-52.060113-51.816386-191.813244-43.383428-349.797164 26.51751a469.272198 469.272198 0 0 0-35.389178-1.316126c-216.819647 0-398.73757 149.209743-448.994102 350.235873C179.870615 322.065892 251.477643 258.940568 347.06742 212.924887c-8.676686 8.140486-59.420672 58.543254-67.902376 67.073704C27.05371 532.012434-52.401331 861.190283 33.098143 946.787248c65.026396 64.97765 182.844086 54.00993 318.210129-12.283847a460.644258 460.644258 0 0 0 209.605324 50.110296c203.268419 0 375.486002-130.783973 437.880145-312.994368h-250.307753a203.755873 203.755873 0 0 1-179.139434 106.752479 203.755873 203.755873 0 0 1-179.090688-106.752479 204.730781 204.730781 0 0 1-24.128985-96.418449v-0.779927h406.439347zM366.468099 452.313665a194.981697 194.981697 0 0 1 194.396751-183.721503 195.030442 195.030442 0 0 1 194.494243 183.770249H366.468099z m577.340804-367.248025c35.291687 35.632905 34.365524 101.244246 4.240852 183.136558a464.446401 464.446401 0 0 0-215.552266-176.458435c94.858595-40.653684 171.973856-46.01568 211.311414-6.678123zM98.368266 930.555022c-45.040772-45.040772-31.440799-139.558149 26.517511-253.476206a464.348911 464.348911 0 0 0 197.126495 241.192359c-100.415574 45.576972-182.600359 53.278749-223.644006 12.283847z" fill="#999999"></path></svg>
            </div>
          </Tooltip>
          ...
          <Tooltip
              ...
              :is-dark-mode="isDarkMode"
          >
            <div class="upload-btn" @click="renderMermaid = !renderMermaid">
              <svg class="upload-icon" :class="{'red': renderMermaid}" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" width="16" height="16"><path d="M959.136 143.296A455.808 455.808 0 0 0 512 442.944c-67.424-186.24-249.184-308.096-447.136-299.648a456.096 456.096 0 0 0 198.4 395.52 244.704 244.704 0 0 1 106.112 202.24v140.032h285.312V741.12a244.704 244.704 0 0 1 106.08-202.24 455.904 455.904 0 0 0 198.4-395.52z" fill="#999999"></path></svg>
...
const CodeBlockModal = defineAsyncComponent(() => import('../components/modals/CodeBlockModal.vue'));
const TableModal = defineAsyncComponent(() => import('../components/modals/TableModal.vue'));
...
// 工具类：编辑器滚动时同步到预览区（绑定到编辑器的scroll事件）
const handleEditorScroll = () => {
  syncScroll(editorRef, previewRef, isSyncing);
};

// 工具类：预览区滚动时同步到编辑器（绑定到预览区的scroll事件）
...
watch(isHtmlMode, (curMode) => {
  nextTick(() => {
    ...
    // console.log('renderMermaid', renderMermaid.value);
    if(renderMermaid.value) {
      renderMermaidCharts(previewRef);
    }
  });
});

// 监听图表渲染
watch(renderMermaid, (renderStatus) => {
  nextTick(() => {
    if(renderStatus){
      renderMermaidCharts(previewRef);
    } else {
      unRenderMermaidCharts(previewRef);
    }
  });
});
